...
static bool _init_gap(const char * name){
    ...
    if(esp_ble_gap_set_device_name(name)){
        Serial.println("gap_set_device_name failed");
        return false;
    }
    ...
    if(esp_ble_gap_register_callback(_on_gap)){
        Serial.println("gap_register_callback failed");
        return false;
    }
    return true;
}
...
MySimpleBLE::MySimpleBLE()
{
    local_name = "esp32";
}

MySimpleBLE::~MySimpleBLE(void)
{
    _stop_gap();
}

bool MySimpleBLE::begin(String localName)
{
    if(localName.length()){
